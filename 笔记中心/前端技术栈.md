...
​		②**我们在谈论对象这个身份：作为对象本身，它像实例对象一样，自然拥有两个属性：**
...
**源码见“..\\..\MYNOTEBOOK\前端\代码笔记\手撸Promise”**
...
(function (window) {
    ...
    class Promise {
        ...
        //promise 原型对象上的方法
        then (onResolved = (data) => { return data }, onRejected = (err) => { throw err }) {}
...
```
创建
eNode = document.createElement("div")			:创建元素节点
tNode = document.createTextNode(string)			:创建文本节点
aNode = document.createAttribute(string)		:创建属性节点
...
属性节点插入
attrNode= document.createAttribute(string)		:创建属性节点   attrNode.value = 123 设置属性值
...
offset系列属性					作用
	...
	element.offsetTop		返回元素相对带有定位的父元素上方的偏移 top:  父元素padding
	element.offsetLeft		返回元素相对带有定位的父元素左侧的偏移 left: 父元素padding
	element.offsetWidth		返回自身(包括padding、边框、内容区)宽度 返回的数值没有单位
	element.offsetHeight	返回自身(包括padding、边框、内容区)高度 返回的数值没有单位
```

![15010945-98b6080ec041e497](../img/15010945-98b6080ec041e497.webp) 
...
<script>
    ...
    console.log(Son.offsetParent === Fa);  //true
    console.log(Son.offsetTop);             // 10
    console.log(Son.offsetLeft);            // 10
    console.log(Son.offsetWidth);           //140
...
client系列属性	作用
	...
	element.clientWidth		返回自身(包括padding、内容区)的宽度 返回的数值不带单位
	element.clientHeight	返回自身(包括padding、内容区)的高度 返回的数值不带单位
```

![15010945-9b760e467b4ab6b5](../img/15010945-9b760e467b4ab6b5.webp) 
...
<script>
    ...
    console.log(Son.clientTop);             // 10
    console.log(Son.clientLeft);            // 10
    console.log(Son.clientWidth);           // 120
</script> 
```
...
scroll系列属性	作用
	...
	element.scrollWidth		返回自身实际的宽度，返回数值不带单位
...
<script>
    ...
    console.log("内容本身的宽度", box.scrollWidth); //800
    console.log("内容本身的高度", box.scrollHeight); //500
    console.log("滚动条滚动之后被卷去的左侧距离", box.scrollLeft); // 0
    console.log("被卷去的上侧距离", box.scrollTop); //0
</script>
```
...
```js
boolean hasOwnProperty(attr)  : // 返回一个布尔值，指示对象自身属性中是否具有指定的属性（也就是，是否有指定的键）
boolean isPrototypeOf(attr)   : // 测试一个对象是否存在于另一个对象的原型链上
...
```js
let id = 1 ,name="CSS"
let a = `<ul><li><p id=${id}>${name}</p></li></ul>`
```
...
原型上的 this 指向实例对象(基于function 而不是箭头函数)

```js
Quene.prototype.push = function (data) {
    this.quene.push(data)      //函数被实例化后，this指向实例对象，因为隐式绑定
}
Quene.prototype.pop = function () {
    return this.quene.shift()  //函数被实例化后，this指向实例对象,因为隐式绑定
}
```
...
原型链继承 + 构造函数继承
	原型链继承
		子类的原型对象指向父类的原型对象
    借用(构造函数继承)(call继承)：本质是通过call或apply实现
...
let Fa = new Father("wang")
let son = new  Son('li',18)
son.intro()   // li 18
```
...
function Circle(center,radius){
    ...
    // 对象方法
    this.getArea = () =>{
        console.log("对象方法_求面积="+3.14*this.radius*this.radius);
    }
}
// 原型方法
Circle.prototype.getLength = function (){
    console.log("原型方法_求周长="+2*3.14*this.radius);
}
// 类方法
Circle.show = ()=>{
    console.log("Circle 类本身"+this.arg);
}
Circle.show2 = function(){
    console.log("Circle 类本身2"+this); // 打印Circle构造器
}
let cir = new Circle(3,1)
cir.getLength()   //原型方法_求周长=6.28
cir.getArea()    // 对象方法_求面积=3.14
this.arg = '@'
console.log(this); // { arg: '@' }
Circle.show()    // Circle 类本身,this指向全局 @
Circle.show2()   // Circle 类本身2function Circle(center,radius){...}
```
...
class Circle {
    ...
    // 类方法
    static show(){
        console.log("Circle 类本身2"+this); // 打印Circle 类
    }
}
...
super 
	1. 在单继承中必须被掉用，否则报错。
	   且会调用了父类的构造器函数，Circle.call(this,xindex,yindex)
	2. super 调用父类方法
```
...
class Circle_son extends Circle{
    constructor(x,y){ 
        super() ;// 调用父类的constructor(x, y) 【且this指向当前】
				// Circle.call(this,xindex,yindex) or Circle.apply(this,[xindex,yindex])
        this.center = x
        this.radius = y
    }
    // 重写父类方法
    info(){
        return super.info()   //  调用父类的 info() 且this指向当前
    }
}
```
...
```
http 是无状态的协议，是指协议对于交互的场景没有记忆能力。
```
...
<img src="../img2/v2-bf990b661bcff759a17ce344416c7b15_1440w.webp" alt="v2-bf990b661bcff759a17ce344416c7b15_1440w" style="zoom:40%;" /> <img src="../img2/v2-64cd282758794c3f77add4ffdaa4457e_1440w.webp" alt="v2-64cd282758794c3f77add4ffdaa4457e_1440w" style="zoom: 40%;" /> 

### URL

即“$\textcolor{red}{统一资源定位符}$”

```
协议://主机名:[端口]/paths[[/params]|[?query]]#hash
```
...
​		**源码见“..\..\MYNOTEBOOK\前端\代码笔记\httpCache”**

### 缓存重要性

```
问题：
	互联网中相同的资源重复请求会带来浪费宽带及网络延时的问题。

处理思想：
	对于相同的资源应该只进行一次请求即可。使用缓存技术
...
```
例子：
	代理缓存、浏览器缓存、网关缓存、负载均衡器、内存分发网络CDN
分类：
	共享缓存 ： 公司内部的web 代理
	私有缓存
```
...
```
强制缓存使用的响应头：
	expires : web ,14 Fed 2022 09:12:59 GMT  // 过期时间
	cache-control: 
```
...
// serve endian code
res.writeHead(200,{
     'Expires': new Date('2022-10-15 10:45:00').toUTCString()
})

功能：
	设置当前资源的缓存时间，当浏览器对应的该资源超过这个时间后，才会再次向服务器获取该资源。
漏洞：
	浏览器和服务器的时间如果不一致就是问题。
```
...
```js
时间：
	http1.1 提供的功能，用来解决强制缓存expires的漏洞，代替expires。同时也可以做协商缓存。
// serve endian code
res.writeHead(200,{
	'Cache-Control': 'max-age=120',   // 单位秒
})

功能：
	在获取资源指定的max-age秒之后该资源失效。
```
...
'Cache-Control': 'no-store' 禁用缓存，与no-cache互斥，不能同时使用。
🐅
'Cache-Control': 'private'  明确该资源只能被浏览器缓存【默认】
'Cache-Control': 'public'   明确该资源可以被代理服务器花奴才能
🐅
'Cache-Control': 'max-age'  明确缓存在客户端上的缓存时间
'Cache-Control': 's-maxage' 明确缓存在代理服务器上的缓存时间
```

```
'Cache-Control': 'public , max-age=120 ,no-cache'
```
...
```
含义：
	在使用缓存之前，与服务器进行一次通信，确定本地缓存的资源是否可用。
```
...
```
客户端请求协商请求携带的请求头，值为对应缓存资源的由服务器给定的资源修改时间戳
```

##### 处理逻辑

```js
// 验证请求头中携带的if-modefied-since参数，如果资源没有改变就返回304状态码
if(pathname === "/imgs/03.jpg"){
    console.log("get 03.jpg");
    const data = fs.readFileSync("./imgs/03.jpg")
    const {mtime } = fs.statSync("./imgs/03.jpg")
...
```
一：
	根据资源的修改时间戳会有问题：资源编辑但未修改，编辑时间会更新
二：
	当资源被急速的修改
本质：
	服务器无法更具资源的修改时间判断资源是否被改变。
```
...
```js
else if (pathname === "/imgs/04.jpg") {
    console.log("get 04.jpg");
    const data = fs.readFileSync("./imgs/04.jpg")
...
```
Etag 只是 last-modified 的补充方案，它任然有一些问题
1. Etag 计算开销
2. Etag 分强验证和弱验证(强验证根据资源内容生成，弱验证根据资源的某些属性生成)
```
...
缓存策略树：
	...
	8. no - private
	9. yes - public
	10. 配置强缓存过期时间 -expires
	11. 配置协商缓存 - Etag / last-modified

缓存处理：
	1. 将网站的资源进行类型拆解
		html 主文件 ： 	协商缓存
		图片文件：	   强制缓存+短缓存时间maxage
		样式表文件：	  强制缓存+长缓存时间maxage+样式版本号变更 
		js文件：		强制缓存+长缓存时间maxage+文件指纹+private
...
<img src="../img2/v2-0489444034d569b37867e2e527a7d5d4_1440w.webp" alt="v2-0489444034d569b37867e2e527a7d5d4_1440w" style="zoom:80%;"  />  
...
```
1. 获取URL对象的响应数据对其进行解析
2. 浏览器开辟一块栈内存，同时分配一个主线程对响应数据进行一行行的解析和执行
3. <!DOCTYPE html> 决定着浏览器的渲染模式
4. 解析HTML,生产DOM树
5. link/script/img等请求后，浏览器将另外开辟新的空间和线程对其进行加载-->TaskQueue
6. 解析CSS，生成CSSOM树
7. DOM+CSSOM = RenderTree
8. Layout
9. 重绘
10.GPU显示
```

<img src="../img2/2022-10-16_210224.jpg" alt="2022-10-16_210224" style="zoom: 80%;" /> 
...
2. 避免重绘和回流
	1. 放弃传统的dom操作时代，基于React/Vue开始数据影响视图模式
		mvvm / mvc / diff / virtual dom / 
	2. 分离读写操作
		offsetXxx ; clientXxx ; scrollXxx ;
	3. 样式集中改变
	...
	5. 元素批量修改
 		文档碎片  /  字符串拼接
 	6. 动画效果使用position；absolute/fixed;(脱离文档流)
 	7. CSS3 硬件加速（GPU)
     	   transform \ opcatity \ filters 会触发硬件加速
     	   
	8. 牺牲平滑度换取速度
	9. 避免使用table布局和使用css的javascript表达式
```
